...
#Define ACT-R functions for activation, recall probability, and retreival time
...
create_tasklog <- function(behavior_log, chunk_list){
    ...
    for(j in 1:4){
      ...
      if(j == 1){
        ...
        if(j == 2){
          ...
          task_log$encode_time[[count]] <- behavior_log$trialStartTime[[i]] + 21.6
        }
  ...
  for(i in 1:nrow(task_log)){
    if(task_log$state[[i]] == "respond" | task_log$state[[i]] == "ignore"){
      ...
      curr_chunk1 <- task_log[[i-2,10]]
      curr_chunk2 <- task_log[[i-1,10]]
...
ind_error_n <- runif(num_runs, 0.2,0.4)
ind_error_l <- runif(num_runs, 0.4, 0.6)

#Create separate error data frames for novel and learned.
for(i in 1:num_runs){
  col_num <- 1 + i
  error_values_l[col_num] <- rnorm(144, mean=0, (pi)^2/3) * ind_error_l[i]
  colnames(error_values_l)[col_num] <- paste0("error",i)
  error_values_n[col_num] <- rnorm(144, mean=0, (pi)^2/3) * ind_error_n[i]
...
ggplot() + geom_point(data=base_rt, aes(trial,rt, colour = cond)) + ylim(0.2,0.4) + labs(x = "Trial", y = "Reaction Time (s)") +
  ...
  ggtitle("Model Reaction Times with No Noise") + scale_fill_discrete(name = "Condition")
#This rt is for two key presses!! I need to divide by 2 to get my average rt per keystroke, which lines up with d
...
mean_rt_full <- data.frame(trial = c(1:72))
for(i in 1:num_runs){
  mean_rt_full[[i + 1]] <- create_meanrt(data_list[[i]])
  colnames(mean_rt_full)[[i+1]] <- paste0("rt", i)
}

#Divide by 2 for average single response
plotting_data <- mean_rt_full[,-1]
...
save(behavior_log, file="behav_log_run6.Rda")
save(task_log, file="task_log_run6.Rda")
save(plotting_data, file="plotting_data_run6.Rda")
save(summary_data, file="summary_data_run6.Rda")